...
while True:
    ...
    batch_states.append(torch.cat(state_list, dim=0))                 # [T, obs_dim]
    batch_actions.append(torch.tensor(action_list, dtype=torch.long, device=device))  # [T]
    batch_returns.append(G_t)                                         # [T]
    batch_ep_count += 1
    ...
    if batch_ep_count < NUM_BATCHES:
        continue
    
    # Batch all operations together
    states_batch = torch.cat(batch_states, dim=0).to(device)  # Shape: (episode_length, state_dim)
    actions_batch = torch.cat(batch_actions, dim=0).to(device)  # Shape: (episode_length,)
    returns_batch = torch.cat(batch_returns, dim=0).to(device)
    ...
    loss = -(returns_batch * log_probs).mean()  # or .sum() depending on preference
